...
static uint16_t get_le16(const uint8_t* p) {
  uint16_t val; std::memcpy(&val, p, sizeof(val));
#if __GNUC__
...
#define CONSUME_ZERO_BYTE_LENGTH \
  ...
  { \
    ...
    for (;;) { NEEDS_IN(1) if (*inp != 0) break; ++inp; } \
    offset = inp - old_inp; \
  ...
  while (true) {
    ...
    if (inst & 0xC0) {
      ...
      if (state == 0) {
        ...
        NEEDS_IN(1)
        nstate = inst & uint8_t(0x3);
        distance = ((inst >> 2) + (*inp++ << 2) + 2049);
        lblen = 3;
      }
    }
    if (distance > std::size_t(outp - dst)) {
      dst_size = outp - dst;
