...
当前优先保证游戏流程和原版渲染行为。任意分辨率、帧率解锁、HDR 和现代超分辨率属于未来规划，不是现有功能。
...
| 存档与调试 | 开发版手动保存已确认。F1 支持战斗判胜；本地传送功能另行跟踪。 |
| 稳定性 | 已有文件日志和 GPU 停帧诊断。营地卡死尚未明确修复。 |

部分遇敌、存档和传送改动仍**仅在本地、尚未提交**。上述结果描述开发工作区，不是干净检出的完整保证，详见[状态页](docs/STATUS.md)。
