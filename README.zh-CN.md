...
**v0.2 正在准备，尚未发布。** 本版加入已核对的欧美四盘版本、按版本提供的游戏及语音语言选项、先导入后首次设置，以及自动读取已导入盘。四盘全部导入后无需手动换盘。两版均已通过原版管理器受控换盘测试；章节交界剧情与完整通关仍未验证。详见 [v0.2 候选发布说明](docs/RELEASE-v0.2.md)。
...
v0.2 候选版已加入经核对的 **USA/Europe 0.0.0.3 四盘版本**，严格校验 XEX 并阻止不同版本混装。游戏语言按安装版本提供：欧美版为英／日／德／法／西／意，亚洲版选项保持不变；设置界面仍保留现有五种翻译。**此支持尚未包含在 v0.1 中。** 详见[欧美版支持与验证](docs/notes/europe-support.md)。

v0.2 候选版在原游戏请求下一盘时自动读取已导入的对应盘，无需玩家点击换盘按钮。四盘全部导入后无需玩家手动换盘。两种已核对版本均通过存储测试及原版管理器 1 → 2 → 3 → 4 → 1 受控流程。章节交界剧情尚未验证，此功能不在 v0.1 中；详见[自动选盘证据](docs/notes/disc-selection.md)。
