...
更新器会检查 GitHub 最新 Release。数字版本更高时按通常规则更新；数字版本相同但 `-后缀` 不同（例如 `0.5.0-hotfix1`）时也会触发更新。更新器策略的源码目标为 0.5.1，尚未包含在已发布的 v0.5.0 程序中。v0.5.1 Release 发布后，现有 v0.5.0 程序可先按数字版本升级；之后的后缀变化由新策略处理。
