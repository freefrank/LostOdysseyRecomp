...
已发布 v0.3.0 中 DLSS／帧生成是**禁用占位**，v0.4.0 开发版已移除这些控件。内部分辨率控制和更高帧率已在开发构建中提供；HDR 及 Linux / Vulkan 游戏运行仍为后续目标。

## 开发状态

**v0.4.0 正在开发。**新增按输出分辨率绘制的设置页文字、双语 Debug 控件、SMAA 1x、可选的实验性相机重投影 TAA、标准／高质量自动空间缩放及 30/60/120 FPS 控制。此前 v7 的选定测试和实际 Map2 移动已验证 TAA 执行链及后续 UI 免重复 AA；该构建的 guest 渲染仍为 720p。不支持的 TAA 路径回退至 SMAA。Debug 切换已在实际 Map2 教学状态通过；图形页预览、超时回退、Keep 和同进程重开也已通过，包括 TAA 与 60 FPS 选择；60 FPS 实现已通过限定移动、对白、菜单及 Ring 核心计时检查，静止营地平均达到 59.80 FPS。精准 Ring 释放／Perfect 和更多流程列为回归，不宣称全游戏锁 60；可选 120 FPS 目标可延期。DLSS/FSR 可行性研究已完成，供应商后端及缺失的原生运动／色彩空间输入保留为后续工作。见[开发证据与边界](docs/notes/v0.4.0-development.md)；改动仍为 Unreleased 开发内容，无新增用户验收，公开下载仍为 v0.3.0。

最新后续开发改动：

- **Shader 发现：**亚洲和美欧联合元数据按导入资源自动匹配，无需区域设置。两版均命中 52 个资源文件索引，CPX 提取零回退，20,686 个来源与完整扫描基准一致，新包通过两版限定 Map2 检查。快速发现仅读所需内容；未知布局保留回退及显式完整扫描模式，不代表整套资源完整性或全游戏验证。
- **内部分辨率：**Auto 跟随输出、最高 4K，另有手动 720p／1080p／1440p／2160p。有界 Map2 检查验证实际场景／深度／TAA 尺寸及细节改善；build-v3 图形页预览、回退和 Keep 通过。
- **实验性 TAA：**修正投影无穷远边界处的无效相机参考点；原生 720p 和 Auto 4K 实测各 256 条记录全部复用历史。
- **Issue #5：**修复两个缺失调用入口，56 项生成函数检查通过；原报告战斗仍待复测。
- **Issue #6：**增加启动内存诊断，186 项注入检查通过；原始根因仍未知。

改动仍为 Unreleased 状态，新版本视觉验收与更广覆盖待完成。见[后续概括、开发包与证据](docs/notes/handoff-v0.4.0-followup.md)。
