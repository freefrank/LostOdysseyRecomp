...
已发布 v0.3.0 中 DLSS／帧生成是**禁用占位**，本地 v0.4.0 开发已移除这些控件。更高内部渲染分辨率、帧率解锁、HDR，以及 Linux / Vulkan 游戏运行仍属于开发目标。

## 开发状态

**v0.4.0 正在本地开发。**新增按输出分辨率绘制的设置页文字、双语 Debug 控件、SMAA 1x、可选的实验性相机重投影 TAA、标准／高质量自动空间缩放及 30/60/120 FPS 控制。选定测试和实际 Map2 移动已验证 TAA 执行链及后续 UI 免重复 AA；guest UI 仍按 720p 渲染。不支持的 TAA 路径回退至 SMAA。Debug 切换已在实际 Map2 教学状态通过；图形页预览、超时回退、Keep 和同进程重开也已通过，包括 TAA 与 60 FPS 选择；60 FPS 实现已通过限定移动、对白、菜单及 Ring 核心计时检查，静止营地平均达到 59.80 FPS。精准 Ring 释放／Perfect 和更多流程列为回归，不宣称全游戏锁 60；可选 120 FPS 目标可延期。DLSS/FSR 可行性研究已完成，供应商后端及缺失的原生运动／色彩空间输入保留为后续工作。见[开发证据与边界](docs/notes/v0.4.0-development.md)；改动仍为本地开发，未推送／未发布，无新增用户验收，公开下载仍为 v0.3.0。
