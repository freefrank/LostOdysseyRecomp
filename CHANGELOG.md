...
- Fix shadow-map rendering across all aspect ratios and high internal resolutions by correcting effective-height render target caching to avoid unnecessary 640×640 recreation and updating depth rasterization without color writes to cover modes 4 and 5 while preserving `SV_Depth` and alpha. In user testing of the affected scene, shadows were confirmed fixed. A separate local CPU usage increase was traced to background system activity (`bun`) rather than game code.
- Note on same-version reissue: A corrected v0.6.6 release is preparing to include this shadow fix. Initial published v0.6.6 packages (commit `c953bb5`) lack this fix and are being superseded; the fix is not present in existing downloads until the refreshed release assets are published by CI.
...
- 修复全显示比例与高内部分辨率下的阴影贴图（shadow map）渲染：修正 effective-height 渲染目标缓存以避免不必要的 640×640 重新创建，并在无颜色写入的深度光栅化中覆盖模式 4 与 5，同时保留 `SV_Depth` 与 alpha。受影响场景经用户实机测试确认阴影已恢复正常；此前汇报的本机 CPU 占用上升经排查为后台系统进程（`bun`）所致，非游戏回归。
- 同版本重新发布说明：包含该阴影修复的 v0.6.6 修正版本正在准备发布。初版已发布的 v0.6.6 资产（commit `c953bb5`）尚未包含该修复并将被替代；在 CI 完成新包构建与发布前，已有下载不包含此项修复。
