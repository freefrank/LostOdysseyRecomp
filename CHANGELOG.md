...
- Build guidance: clarified how to reconcile the actual modified XenonRecomp tree after a tracked patch update, then rebuild generator tools, regenerate PPC and rebuild the runtime; an incremental runtime build or codegen check alone does not prove the tree matches the patch. In a bounded local Windows physical-controller check, 2× hold visibly accelerated and release restored normal speed after the local clock source was synchronized (close the debug menu, release LT, then press/hold again to test). The tracked patch already contained the correction; this is not a newly released runtime fix or broader platform/gameplay validation.
- Cheats sidebar: LT selects the previous category and RT the next, wrapping at either end and moving only once per press (not repeatedly while held); added an on-screen English/Chinese control footer. Windows `cheats_ui` regression passed (1/1), and a physical-controller check confirmed switching, no hold-repeat, and the footer; other platforms and full gameplay were not validated.
- Settings menu DLSS and FSR quality order: DLSS quality choices and left/right cycling now order as Performance, Balanced, Quality, and DLAA, with FSR placing Native AA last. Persisted configuration values, defaults, and GPU runtime behavior remain unchanged. Rebuilt standalone `LoMenuFlowTest` checks passed across all 8 test groups. In-game verification and release inclusion remain pending.
- Settings menu Quit to Main Menu: added a "Quit to Main Menu" action at the bottom of the System -> Settings -> Game page. The confirmation prompt defaults to Cancel, back input cancels, and confirming waits for retail Settings close completion before invoking the original title transition on the same guest thread (`RequestMainMenuAfterSettingsClose`), returning directly to the title screen without posting `SDL_QUIT` or saving settings. Menu entries and confirmation dialogs support English, Traditional Chinese, Simplified Chinese, Japanese, and Korean. Rebuilt standalone `LoMenuFlowTest` passed across all 8 test groups, covering mouse selection, gamepad confirmation, Cancel defaults, localization, single helper invocation after Settings close, and absence of `SDL_QUIT`. Verified through synthetic menu event fixtures only; real-hardware execution, cancellation, and dialog display remain unconfirmed by users.
- Native System menu Quit to Desktop (Issue #35, physical-hardware quit accepted, unreleased): swapped the native System menu quit entry to "Quit to Desktop", overriding label ID 9104, help ID 9114, and prompt ID 9120 across all nine supported game languages. Trailing garbage characters observed in in-game captures were resolved by including the UTF-16 null terminator in the descriptor copied code-unit count (`text.size() + 1`), preventing downstream consumers from reading uninitialized memory. Investigation into an initial return-to-title defect on v2 builds revealed the live guest return address was `0x822E256C` (System exit task), whereas the initial hook matched only `0x822E26B8` (modal-16 dialog); v4 matches both callers and handles `SDL_QUIT` dispatch failure recovery for each continuation. Standalone CPU contract tests `LoQuitTextHookTest` and `LoQuitActionHookTest` (independently verifying the generated PPC callsites and continuations) passed. In physical-hardware testing on the verified v4 build (`ffb59dbc5714db0d0eaced275d444475b8369b5e6fe91f2b94a12d8128c008b5`), the user explicitly accepted that selecting and confirming "Quit to Desktop" in the System menu successfully exits the process. Acceptance covers this specific System quit action path only; Settings menu return to Title, other UI languages, and debug Save Anywhere remain unverified on hardware item by item. The change is unreleased, and Issue #35 as a whole is not claimed closed.
- Live anisotropic filtering (merged source, unreleased): added Off / 2x / 4x / 8x / 16x to Graphics; saving applies the supported device-clamped level without a restart. Immutable sampler and descriptor table generations remain pinned through GPU batch completion, while excluded textures keep their guest sampling. Windows/Linux asset-free CPU and settings-menu fixtures plus software Vulkan descriptor isolation and readback with validation passed in Actions run 36079262997. Full-game, physical D3D12/Vulkan, Steam Deck, image-quality, and frame-time checks remain pending. See [AF notes](docs/notes/anisotropic-filtering.md).
- Debug save anywhere persistence (Issue #61): preserved the debug "Save Anywhere" toggle across sessions without adding it to the player settings menu. The setting defaults to disabled. Toggling it in the debug menu persists immediately while keeping unconfirmed graphics previews intact. Windows standalone test `LoSaveAnywhereConfigTest` passed configuration handling, cross-process restoration, and preview coexistence. In-game validation remains pending, and this does not fix the existing camera offset when reloading saves created this way.
...
- 构建说明：明确拉取 XenonRecomp 补丁更新后先核对并同步实际已修改的依赖工作树，再依次重建生成器工具、重新生成 PPC、重建运行时；仅增量构建运行时或执行代码生成检查，不能证明工作树已与补丁一致。本地 Windows 实体手柄的限定验证中，同步本地时钟源码后，按住 2× 明显加速、松开恢复正常（测试时先关闭调试菜单、松开 LT，再重新按住）。受跟踪补丁此前已包含此修正；不表示本次新增或发布了运行时修复，也不代表其他平台或完整游戏流程已验证。
- 作弊菜单侧栏：LT 切到上一分类、RT 切到下一分类，首尾循环，每次按下只切换一次（长按不连跳）；增加中英双语屏幕操作提示。Windows `cheats_ui` 回归测试通过（1/1），实体手柄确认切换、长按及提示正常；尚未验证其他平台或完整游戏流程。
- 设置菜单 DLSS 与 FSR 画质档位顺序：DLSS 画质显示与左右循环调整为 Performance、Balanced、Quality 与 DLAA，FSR 对应将 Native AA 置于最后。已保存的配置值、默认设置与 GPU 运行时行为保持不变。重建后的离线 `LoMenuFlowTest` 全部 8 组检查均已通过。实机游戏验证与发布纳入仍待进行。
- 设置菜单退出到主菜单：在系统菜单的设置→游戏页末尾新增“退出到主菜单（Quit to Main Menu）”选项。确认对话框默认选中取消，返回键直接取消；确认后等待原版设置应用与关闭流程完成，再在同一 guest 线程通过 `RequestMainMenuAfterSettingsClose` 直接调用原版实现返回标题画面，不投递 `SDL_QUIT` 且不触发设置保存。选项与确认对话框支持英文、繁体中文、简体中文、日文和韩文。重建后的离线 `LoMenuFlowTest` 全部 8 组检查均已通过，覆盖了鼠标选择、手柄确认、默认及返回取消、多语言文案、设置关闭后单次触发原版返回以及无 `SDL_QUIT` 投递。目前仅通过离线事件测试验证，实机上的实际跳转、取消操作及对话框显示仍待用户逐项确认。
- 原生系统菜单退出到桌面（Issue #35，实机退出已验收、未发布）：将原生系统菜单退出入口调整为“退出到桌面（Quit to Desktop）”，覆盖文本标签 ID 9104、帮助信息 ID 9114 以及确认提示 ID 9120 在全部九种语言中的显示。通过将描述符 +12 处的复制长度修正为 `text.size() + 1` 包含 UTF-16 终止符，消除了此前实机截图反馈中文案尾部的乱码字符，避免下游消费者深拷贝未初始化内存。针对此前 v2 实机确认后仍返回主菜单的缺陷，排查 v3 日志确认实机运行时实际返回地址为 `0x822E256C`（系统任务退出），而旧 hook 仅匹配了 `0x822E26B8`（modal-16 对话框）；v4 精确匹配这两个 caller 并分别处理 `SDL_QUIT` 派发失败恢复。离线 `LoQuitTextHookTest` 与静态比对生成 PPC 调用点续段的 `LoQuitActionHookTest` 均已通过。经已校验部署哈希（`ffb59dbc5714db0d0eaced275d444475b8369b5e6fe91f2b94a12d8128c008b5`）的 v4 完整构建实机测试，用户明确验收原生系统菜单确认退出到桌面已成功退出进程。本次实机验收仅涵盖该原生退出动作路径，设置菜单返回标题、其余各语言显示及随时存档等尚未逐项实机验收。改动尚未发布，Issue #35 整体不声称关闭。
- 实时各向异性过滤（AF，源码已合并、未发布）：图形设置增加关闭 / 2x / 4x / 8x / 16x；保存后按设备支持上限应用，无需重启。采样器与描述符表的不可变版本保留至 GPU 批次完成；不符合条件的纹理沿用原版采样。Windows/Linux 无游戏资源 CPU 与设置菜单测试，以及带验证层的软件 Vulkan 描述符隔离与读回，在 Actions 运行 36079262997 中通过。完整游戏、实体 D3D12/Vulkan、Steam Deck、画质与帧时间检查仍待进行。见[AF 记录](docs/notes/anisotropic-filtering.md)。
- 调试随时存档状态保存（Issue #61）：在重启后记住调试菜单中的随时存档（Save Anywhere）开关状态，且不将其加入玩家设置菜单。该项默认关闭。在调试菜单切换时立即写回配置，且不会提交未确认的图形预览。Windows 独立测试 `LoSaveAnywhereConfigTest` 通过了配置解析、跨进程恢复以及与图形预览共存的检查。实机验证仍待推进，且本改动不修复此类存档读档后的镜头偏移问题。
- 输入与窗口体验：
