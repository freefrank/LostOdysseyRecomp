...
// Host <-> guest call bridging. A host function `R f(A...)` is exposed to the
// guest by GUEST_FUNCTION_HOOK(sub_xxx, f): integer/pointer arguments come from
// r3..r10 then the stack, floating point from f1..f13, and the return value
// goes back into r3 / f1. Pointers are translated relative to the guest base.
// Design follows UnleashedRecomp (GPLv3).
...
struct ArgTranslator
{
    constexpr static uint64_t GetIntegerArgumentValue(const PPCContext& ctx, uint8_t* base, size_t arg) noexcept
    {
        ...
        // Stack arguments: 32-bit values in 8-byte slots after the 0x50 byte
        // home area (0x54 is the low word of the first slot).
    ...
    constexpr static void SetIntegerArgumentValue(PPCContext& ctx, uint8_t* base, size_t arg, uint64_t value) noexcept
    {
        ...
        assert(false && "Pushing to stack memory is not yet supported.");
    }
    ...
    constexpr static std::enable_if_t<!std::is_pointer_v<T>, void> SetValue(PPCContext& ctx, uint8_t* base, size_t idx, T value) noexcept
    {
        if constexpr (is_precise_v<T>)
            ...
            SetIntegerArgumentValue(ctx, base, idx, static_cast<uint64_t>(value));
    }

    template<typename T>
    constexpr static std::enable_if_t<std::is_pointer_v<T>, void> SetValue(PPCContext& ctx, uint8_t* base, size_t idx, T value) noexcept
    {
        SetIntegerArgumentValue(ctx, base, idx, value ? g_memory.MapVirtual((const void*)value) : 0);
    }
};
...
constexpr std::array<Argument, std::tuple_size_v<T1>> GatherFunctionArguments(const T1& tpl)
{
    ...
    if constexpr (!args.empty())
    {
        std::apply([&](const auto& first, const auto&... rest)
            {
                auto append = [&]<typename T2>(const T2&)
                {
                    if constexpr (is_precise_v<T2>)
                        args[i] = { 1, floatOrdinal++ };
                    else
                        args[i] = { 0, static_cast<int>(i) };
                    i++;
                };
...
PPC_FUNC(HostToGuestFunction)
{
    ...
    else
    {
        ...
        {
            ctx.r3.u64 = v != nullptr ? static_cast<uint32_t>(reinterpret_cast<size_t>(v) - reinterpret_cast<size_t>(base)) : 0;
        }
