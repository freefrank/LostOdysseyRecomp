...
static inline const wchar_t *Translate(uint32_t language, const wchar_t *en, const wchar_t *traditional)
{
    ...
    static constexpr Entry entries[] = {
        ...
        {L"Direct3D 11 (probe)", L"Direct3D 11（プローブ）", L"Direct3D 11(프로브)", L"Direct3D 11（探测）"},
        {L"Direct3D 11 currently probes the adapter then falls back to Direct3D 12 or Vulkan. A renderer is not implemented.",
         L"Direct3D 11 は現在アダプターを検出してから Direct3D 12 または Vulkan にフォールバックします。レンダラーは未実装です。",
         L"Direct3D 11은 현재 어댑터를 탐지한 뒤 Direct3D 12 또는 Vulkan으로 대체합니다. 렌더러는 구현되어 있지 않습니다.",
         L"Direct3D 11 目前只探测适配器，然后回退到 Direct3D 12 或 Vulkan。渲染器尚未实现。"},
