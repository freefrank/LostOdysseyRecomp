...
int main() {
        ...
        inputs.plan.consumer = upscaling::TemporalConsumer::FsrSr;
        Require(!inputs.CompleteForConsumer(), "FSR requires tracked or reset-initialization motion");
        inputs.motionState = MotionState::Tracked;
        inputs.depthConvention = DepthConvention::Unknown;
        Require(!inputs.CompleteForConsumer(), "FSR requires explicit depth convention");
        inputs.depthConvention = DepthConvention::Reversed;
        inputs.motionInvalidity = {};
        Require(!inputs.CompleteForConsumer(), "FSR requires valid motion invalidity region");
        inputs.motionInvalidity = {token, {64, 32}, 0, 0, 64, 32};
        Require(inputs.CompleteForConsumer(), "FSR complete inputs accepted");
        inputs.plan.consumer = upscaling::TemporalConsumer::LegacyTaa;
        inputs.plan.frameGeneration = upscaling::FrameGeneration::Dlss2x;
        inputs.motionState = MotionState::Unavailable;
        Require(!inputs.CompleteForConsumer(), "independent FG requires motion with legacy TAA");
        inputs.motionState = MotionState::Tracked;
        Require(inputs.CompleteForConsumer(), "FG and legacy TAA requirements coexist");
        inputs.plan.frameGeneration = upscaling::FrameGeneration::Off;
