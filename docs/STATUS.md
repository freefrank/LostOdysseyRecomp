...
## Development branch: Native DLSS integration (P1 temporal inputs passed; P2 paused work in progress)
...
- **P2 Scope & Status**: P2 implementation is committed in checkpoint commits `d018be7` (native DLSS execution adapter) and `744ab91` (checkpoint guarded DLSS scene-copy path), and is paused at user request. Gate 3 initial review attempt 1 identified 4 action items (addressing readback SR guard, validation define leakage, JSON bracket formatting, and self-test failure resource cleanup); all four were completed and verified locally (including clean self-test queue draining before texture teardown with 0 resample mismatches). Formal re-review was cancelled at user direction; Gate 3 is not approved, and P2 remains an uncompleted WIP checkpoint. Persistent NGX controller lifecycle, evaluation recording with checked Vulkan reset/begin/end, and retirement tracking passed in `LoNativeDlssExecutionTest` (exit 0, raw NGX 1, raw VK 0). Renderer SR routing, monotonic submission serials, unknown color bypass, and destination target promotion architecture with parked low-res mappings and `p2-oracle.jsonl` capture hooks are implemented in working tree. Production destination resample was validated on an RTX 5080 via `--self-test-scene-copy-promotion` (4×4 to 8×8 readback pass across 64 RGBA destination pixels with 0 mismatches and 1-byte alpha tolerance). Comprehensive handoff and developer guide are documented in [docs/notes/native-dlss-handoff.zh-CN.md](notes/native-dlss-handoff.zh-CN.md).
- **Verification Evidence**:
  ...
  - Standalone P2 native execution fixture (`LoNativeDlssExecutionTest`, `.cache/evidence/native-dlss-p2-a.json`) passed native NGX create/evaluate recording, persistent session sizing, and isolated buffer exclusion. The post-submission check verified only that the first RGBA16F output pixel was finite and nonzero compared to a zero sentinel; it does not prove motion response or color/alpha preservation. Validation layers were unavailable during execution (`VALIDATION_LAYER_KHRONOS=unavailable`).
...
- **Scope Limits**:
  - No actual in-game Super Resolution dispatch has occurred. Static code review confirms the unknown color encoding guard bypasses SR evaluation in-game; this is not runtime game validation, and tonemapped data is unqualified rather than invalid. DLSS cannot be claimed as usable in-game.
  ...
  - Known UNORM storage format does not establish whether the underlying game color encoding is linear or gamma-curve. Offline analysis of target post-processing shader `b4b4d54a7a2d6b96` confirmed the final RGB pass computes `exp2(c10.x * clamped log2(v))` where `c10.x` is a runtime pixel shader constant; runtime PS constants and the producer chain remain unqualified.
