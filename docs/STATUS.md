...
- **Flatpak offline packaging and target-device verification (development branch; release pending)**:
  ...
  - CI integration: Linux release workflow (`.github/workflows/release.yml`) timeout increased to 360 minutes and updated to stage required inputs and upload the generated `.flatpak` bundle, SHA-256 sidecar, and `source.json` artifacts. No Flatpak assets are uploaded to GitHub Releases; the CI workflow has not yet been executed in this task.
  - Verification & user acceptance:
    - Target hardware acceptance: The development bundle `LostOdysseyRecomp-v0.7.1-4343016c-dev.flatpak` (265,619,912 bytes, SHA-256 `8c8c0ca1c1a3987c4bc23850616990c56bf9e3dc4b66e579e60f420f33bd0290`, app ID `io.github.freefrank.LostOdysseyRecomp`, branch `dev`, OSTree commit `f072caee4563c0b66bfda718d3b9d3a1f646e5ec0d54ddb75c0fc9babd6cc26a`, built from `main` `4343016c5ec25d63073843db3a5ab4b6652dba57` with dirty working-tree state) was transferred to target SSH host `psvita` (`/home/freefrank/Downloads/LORecomp-flatpak/LostOdysseyRecomp-v0.7.1-4343016c-dev.flatpak`). The user installed and launched the application under systemd user service `lo-flatpak-device-test.service` (active/running) on Freedesktop runtime 26.08, and explicitly verified and accepted the result ("验证通过"). Specific gameplay scenes and frame rate performance metrics were not measured or reported.
    - Verification boundaries: Local user installation in WSL succeeded, but a test script path validation error incorrectly rejected the valid `XDG_STATE_HOME` directory (`~/.var/app/<id>/.local/state`) and aborted prior to executing `ldd -r`; this check is recorded as aborted/unpassed and is not claimed as verified. Prior 8/8 offline checks and subsequent bounded test cases passed and were not re-tested.
    - Publication status: User confirmed intent to submit a new application PR to `flathub/flathub`, but the PR has not yet been created. No Flatpak packages have been publicly released. The latest public release remains v0.7.0 (available as AppImage and Windows ZIP only).
  - Milestone context & Gate 1 P0: Streamline Frame Generation Gate 1 P0 remains blocked on SDK validation and display evidence; earlier lifecycle fixes are implemented and reviewed (`out/streamline-fg-p0/gate1-triage-next/REPORT.md`), but Gate 1 P0 has not passed.
- **v0.8.0 roadmap targets**:
  - Incorporated D3D12 DLSS Frame Generation and dynamic Multi-Frame Generation (dynamic MFG) into the v0.8.0 roadmap planning scope, retaining the existing target of fixed 2× DLSS FG on Windows Vulkan and an independent FSR Frame Generation target, without predetermining dynamic MFG APIs, platforms, or generation multipliers. Added Flatpak package release planning (progressing in parallel with the frame generation track, without promising Flathub acceptance or publication), Linux AArch64, macOS AArch64 (specifically Apple Silicon, without precluding other macOS architectures), experimental Android support, and removal of the legacy PM4 packet translation layer (promoted from v0.9.0) as roadmap targets for v0.8.0. All newly incorporated items are uncompleted roadmap planning targets only, not existing implementations, verified test coverage, user acceptance, or delivered features.
