...
P2 remains in progress; final visual acceptance is pending. The pushed checkpoint includes three battle shader-slot mappings. Candidate `cb41c99` (source identity `d78d80f3add32494c53f67702d27d52c5c5e23aa82d690339cba046768c6357e`) passed CPU checks for 768 clips; GPU fixtures compiled six mapped microcodes to DXIL/SPIR-V and passed Vulkan replay checks for alpha discard, palette motion, and two-quad geometry. One RTX 5080 Vulkan battle run recorded 2,476 completed FSR uses and no motion-vector first-failure records during that window. This does not establish visual acceptance or whole-game coverage. `02d8` remains unmapped without evidence that it blocks coverage; `f6` is fixture-only and was not observed in the game run. The user-approved APEX 15W proxy covers this round's low-power hardware requirement, but is not equivalent to Steam Deck hardware behavior. See the [current FSR handoff](notes/fsr-dlss-fg-codex-handoff.zh-CN.md) for coverage limits, evidence paths, and next steps.
