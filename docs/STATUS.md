...
## v0.6.15 Release Preparation (2026-09-24) / v0.6.15 发布准备（2026-09-24）

Status as of 2026-09-24:
The codebase is prepared for the v0.6.15 release, consolidating the `FSR` development branch and native DLSS runtime improvements into `main`. The release packages experimental native NVIDIA DLSS 310.9.1 and AMD FidelityFX SDK v1.1.4 (FSR 3.1.4) support with automated workflow packaging and license verification. Windows Clang incremental build linked cleanly (exit 0) for `LostOdysseyRecomp.exe` and `LoShaderPackTool`. User testing confirmed Windows rendering visuals without issue, and user acceptance concluded Linux verification based on execution reports of game linkage and `LoFsrAdapterGpuTest --transient-only` passing on an AMD Radeon 8060S RADV STRIX_HALO environment (`psvita distrobox psbuild`). The candidate portable shader pack (`out/merged-shaders/portable_vk.lospv`, 28,527 shaders) was created with `LoShaderPackTool merge` and verified via `verify-runtime`. Release CI execution and release asset distribution lanes remain in progress; the release is not yet published on GitHub Releases. Broad scene coverage, deterministic production renderer fault injection, and DLSS/FSR frame generation remain experimental and open. See [FSR progress notes](notes/fsr-dlss-fg-codex-progress.zh-CN.md) and [FSR repair notes](notes/fsr-repair-2026-09-24.zh-CN.md) for detailed boundaries.
...
- **Remaining Scope & Boundaries**: The merged pack has not replaced the active game pack, has not been published to release repositories, and is not bundled into release distributions. Microcodes and private logs are excluded from Git. Core and merge unit tests passed; game rebuild and re-testing of passed functionality are not required. Detailed format and usage instructions are documented in [Portable Shader Pack](PORTABLE_SHADER_PACK.md).
