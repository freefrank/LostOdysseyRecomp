...
- [x] **Installer drag crash:** the v0.5.4 release target includes the locally accepted `PostMessageW` repair for the v0.5.3 drag stutter/exit. Four local `ucrtbase` 0xc0000409 events and dump 41648 identified synchronous `SendMessageW` from Tk binding into Python WNDPROC/Tk re-entry, ending in a fatal GIL-state error. The message-only HWND DragDispatch case passed 1/1; the legacy fixture twice failed an unchanged foreground setup assertion before changed behavior. The installer-only replacement EXE is 11,888,743 bytes (SHA-256 `707CD7D2E9F4AB3BF33363E172FAAD5CFCFA6B1A53161FEE0E7F53735B7C7FA7`), and read-only PYZ inspection confirms the intended dispatch/modules. The user accepted the reported path on 2026-09-10. Publication remains pending.
