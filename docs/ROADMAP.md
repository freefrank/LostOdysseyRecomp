...
- [~] **Release PPC-library reuse:** the release workflow now restores a precompiled PPC static library by default, with `rebuild_ppc=true` as the source-build fallback. Export, restore and verification CLI commands plus CMake import support are implemented in [`2b5b1d1`](https://github.com/freefrank/LostOdysseyRecomp/commit/2b5b1d1), pushed to `github/main`. Thirteen synthetic fixtures, actionlint for both workflows, a real 138,454,798-byte PPC library export (SHA-256 `ba3e4c4dff009d6d8e844c007186a6e5040266875bca6423f8fe26f8d27fb21b`), restore/CMake-target checks and a runtime Ninja graph check passed. The [PPC prebuilt synthetic-test run](https://github.com/freefrank/LostOdysseyRecomp/actions/runs/34553414428) completed successfully; it is separate from hosted release verification. The graph contains zero PPC compile commands and links the imported library behind its verification dependency. Private build-inputs commit `77f076e0e03966736cbf8919ce793bafadce82d9` is pushed and its five blobs were read back through the GitHub API; this is artifact transport, not a public release. Hosted release end-to-end CI, a new release publication and player acceptance remain pending.
