...
- [x] **Installer drag crash:** the user reported that the released v0.5.3 installer stuttered and exited when dragged. Four local `ucrtbase` 0xc0000409 events and dump 41648 identified synchronous `SendMessageW` from Tk binding into Python WNDPROC/Tk re-entry, ending in a fatal GIL-state error. The local unpublished repair switches drag dispatch to `PostMessageW` with screen coordinates; the new message-only-HWND DragDispatch case passed 1/1. The same legacy fixture failed twice on an unchanged foreground setup assertion before reaching drag behavior. The installer-only replacement EXE built locally (11,888,743 bytes; SHA-256 `707CD7D2E9F4AB3BF33363E172FAAD5CFCFA6B1A53161FEE0E7F53735B7C7FA7`), and read-only embedded-PYZ inspection confirms the intended dispatch/modules. The user accepted the repair as resolved on 2026-09-10 for the reported drag path. It remains unpublished.
