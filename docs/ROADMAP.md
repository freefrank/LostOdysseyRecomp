...
The earlier v0.4.0 checkpoint completed the recorded implementation and bounded validation for: text/UI, Debug, spatial AA/scaling, experimental TAA, sampled 60-FPS behavior and DLSS/FSR feasibility research. Wider gameplay remains regression coverage; no new user acceptance is recorded. See the [implementation checkpoint and acceptance criteria](notes/v0.4.0-development.md). v0.4.0 remains in development and Unreleased, with no committed release date.
