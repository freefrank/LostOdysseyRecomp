...
- [~] **Release PPC-library reuse:** automatic local-PPC synchronization is implemented and enabled (`lo.ppcAutoSync=true`): local input changes publish an immutable `ppc/<key>` branch, while a matching remote key is reused without rebuilding or uploading. Nineteen sync fixtures, two directly affected extraction cases, both workflow actionlint checks, a real `LoPpcAutoSync` hook upload, sparse-clone/restore/hash and CI-contract checks passed. The real hook uploaded only an existing library to private commit `5e80263491b39dc0012146dd3a31cf5eea533225`, then the same key remained unchanged; it did not compile PPC C++ or run the game. CI now resolves by key without updating a workflow SHA pin; CI/imported/recursive paths cannot upload, and auth/network failures are not cache misses. The implementation is pushed as [`2c0456c`](https://github.com/freefrank/LostOdysseyRecomp/commit/2c0456c) to github/main. Hosted [PPC prebuilt tests](https://github.com/freefrank/LostOdysseyRecomp/actions/runs/34565564964) passed; it is not in the published v0.5.4 package, and no player acceptance is recorded. The earlier manual-path [`2b5b1d1`](https://github.com/freefrank/LostOdysseyRecomp/commit/2b5b1d1) and [synthetic CI](https://github.com/freefrank/LostOdysseyRecomp/actions/runs/34553414428) remain historical evidence.
...
- [~] **Performance and shader startup:** local unpushed `perf-gpu-ring` implements a 2-slot GPU command ring, D3D12 descriptor limit 1800 and texture-set reuse (`b91d279`), then static dummy unused banks, last-hit cache and skipped unchanged constant uploads (`ed90fe9`). Isolated user01 city walking at 1280×720 D3D12 versus published v0.5.4: ring EXE `5917F389…` about 57.7 fps (was 31–44), batches 5.2→2.0, `fence_wait` 15.40→1.67, splits 0; dummy EXE `02E303F1…` `bind_ms` 1.99→1.58 and hits 5289→837 with no further fps gain. Header fixtures 22/22 and 12/12. Remaining stalls, the two retained compiler failures and 60 fps acceptance stay open; this is not in published v0.5.4 and is not player acceptance. A fixed-scene measurement is not the full game. See [GPU-ring compare](notes/perf-gpu-ring-compare.md).
