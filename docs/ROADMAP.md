...
## 阶段 4：现代化
- [x] 按逻辑线程数减1进行多线程预编译：本机15worker，2000着色器准备53.4s→6.7s，成功产物逐字节一致。
- [x] 已知着色器启动准备、默认持久缓存和标题进度；184 个微码的热缓存与损坏 DXIL 重建通过，见 [着色器准备](notes/shader-preparation.md)。
- [x] 首次启动扫描FPD并提取预编译：四盘2000个微码、1998编译成功，扫描/编译进度、缓存复用和独立Map12验证通过。
- [ ] 2个微码编译失败、未覆盖容器/运行时变体和实际图形管线（PSO）预创建；首次遇到未知场景仍可能编译卡顿。
- [x] 新设置入口、原游戏选项桥、五语言界面、语言选择、FXAA 与等比例输出缩放；见 [设置菜单](notes/settings-menu.md)。
- [ ] 新设置的全屏／独占模式、鼠标和跨 DPI 桌面验收。
- [ ] 任意内部渲染分辨率与宽屏，UI 布局修正
- [ ] 帧率解锁，找出写死 30fps 的逻辑
