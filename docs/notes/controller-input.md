...
## IME interference: implemented, physical verification pending (2026-09-24)

A user reported that Z/X/A/S could stop reaching gameplay while an IME was active, and confirmed that switching to an English input method restored controls. The game window now explicitly calls `SDL_StopTextInput()` after creation and reasserts that state after focus regain. This disables SDL host text/IME composition for gameplay without changing the user's global input-language setting. Installer text-entry flows remain separate and continue to opt into `SDL_StartTextInput()` only while naming a destination.

Physical validation under Chinese/Japanese/Korean IMEs, focus changes, and keyboard/controller switching is still pending; the implementation does not claim that every platform IME interception path has been reproduced.
