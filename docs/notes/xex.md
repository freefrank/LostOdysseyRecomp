...
本地开发已支持经核对的 USA/Europe `0.0.0.3` 四盘 XEX 集合；这项支持尚未提交、推送或发布，不在 v0.1 中。四盘完整解密镜像均为 20,709,376 字节，与当前亚洲版 Disc 1 逐字节一致，因此复用现有重编译代码和地址。原始 XEX 哈希及资源包仍有差异，不能混装。
...
后续本地实现已接通原版换盘请求：先验证目标盘 XEX 身份、FPI 盘号及结构、13 个 FPD 和索引范围，再切换游戏路径，原版管理器继续卸载／重载对应索引。每盘仍保留自身 FPI/FPD，没有合并改写成统一索引。目标盘缺失或不合法时保留旧挂载并返回失败。两版存储读回与两版原管理器受控流程均已通过；章节交界剧情尚未验证，见[自动选盘](disc-selection.md)。以下“合并策略”属于早期调查记录，不是当前实现。
