...
## 2026-09-06（本地）：Map 13 连续遮罩证据与首战报告分流

官方 v0.2.2（EXE `ccc63c3f95495ae4d0052ce6d4ee0f62c20897bb17f64bb79079b55a40a75c48`）在亚洲版 Map 13 独立存档副本中再次复现右臂／裤腿单帧暗→亮→暗；600 张连续显示图中，swap 6963、7124、7285、7446 相隔 161 帧出现跳变。宽范围 `LO_NO_DEPTH_FETCH` 干预虽移除大跳，也破坏景深并使场景模糊，仅属诊断，不能当作修复或干净视觉 A/B。

本地新增可选定向连续 readback 诊断 `LO_RESOLVE_TRACE_REQUEST`：在指定 resolve 排队复制，复用已有帧 fence 后映射，不在阴影生成与消费之间刻意插入逐 resolve GPU 等待。320 帧（renderer 1293–1612）取得 1,280 个 raw 文件，覆盖屏幕深度、两次阴影 atlas 与首份阴影遮罩，另有 4,160 条 PS 常量记录；319 帧与显示截图按 `swap = renderer frame + 1` 对齐。复制／barrier 与写盘仍会扰动时序，不能用作卡顿基准。

f1526／1527／1528 裤腿 ROI（x648–671、y480–511）的显示 RGB 均值为 81.83／136.94／86.97，遮罩均值为 23.35／182.20／85.30，闪烁已存在于遮罩。CPU 使用消费前首份 atlas、屏幕深度及捕获常量重算现有翻译算法，在 f1523–1530 八帧 24×32 ROI 中，对 GPU 遮罩的平均绝对误差为 **0.000185–0.000952（[0,1]）**。两组移动格栅投影（零起始 draw 0、3）主导变化，角色投影 draw 2 较平滑；atlas 仍有完整格栅与人物轮廓，不是整张 atlas 被清空。

这将这些像素的变化收窄至移动阴影贡献，但尚未证明原版正确时间行为、所有 shader 指令语义或最终根因，也不能直接定为纯走样。下一步需要原机／Xenia 的时序投影与过滤对照。已验收脚下投影修复与本次身体／环境阴影闪烁保持区分。

首战明暗闪烁来自美版玩家独立反馈；维护者明确自己没有看到，本机尚未复现，不能与 Map 13 合并根因。新独立首战 `out/firstbattle-temporal-01` 正继续采集，此检查点尚无结果。Map 13 完整证据为 `out/map13-temporal-01/report.md`、`mask-correlation.png` 与 `filter-replay.json`。诊断构建通过，主 EXE／PDB 已恢复原哈希（`D584876D`／`E8244EEB`）；本轮只有实验与诊断工具，未验收渲染修复、未提交或推送。


## 2026-09-07 04:02:30 UTC：按十分钟止损条件挂起

用户指定的继续排查窗口为 03:52:30–04:02:30 UTC（本地 2026-09-06）。期限结束仍未确认首战身体阴影闪烁的本机复现或可验证直接原因，已按要求挂起阴影调查并保留证据；这不是修复完成，也不推翻上节 Map 13 独立复现和遮罩证据。

首战两组连续捕获累计 1,200 帧、3,600 个 raw 文件和 6,018 次 PS draw。屏幕／阴影深度均无非有限值或越出 [0,1] 的值，未见整张阴影 atlas 清空、trace 截断或 readback 写入错误。候选检测后的目视检查对应镜头、姿势／头发边缘、背景效果或受击火花，未确认身体阴影 toggle；启发式候选检查不能证明玩家报告不存在。另对 Map 13 做的微小采样 epsilon CPU 敏感性测试未消除跳变，仅排查了一个候选，未得到最终根因。

首战记录：`out/firstbattle-temporal-01/report.md`、`capture-integrity.json`。独立诊断进程 57032 已核对 EXE 路径后停止；原主 EXE/PDB 哈希仍为 `D584876D`／`E8244EEB`。当前已切换到攻击动画卡顿测试 `out/firstbattle-stutter-cold-01`，进行中、尚无结论；该测试使用官方 v0.2.2，不启用连续截图、PS trace 或 resolve readback，不能提前将卡顿归因为 shader 编译。本次无验收修复、未提交推送。
