...
The user has reproduced the startup defect, not accepted the updated fix in gameplay. The complete Issue #4 save-point crash remains unreproduced, and in-game acceptance is pending. The user has authorized merging this fix and including it in v0.2.2 release preparation; the release is not yet published. Automated path and storage tests do not establish gameplay success.
