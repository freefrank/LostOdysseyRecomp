...
> **历史快照 — 2026-09-10。**此保留路线图不是当前计划。相对链接已按 `docs/archive` 重定向；请以[当前路线图](../ROADMAP.zh-CN.md)查看活跃方向和状态。

[English](ROADMAP-2026-09-10.md) · [当前状态（英文）](../STATUS.md)
...
- **版本与交付：**[v0.5.1](https://github.com/freefrank/LostOdysseyRecomp/releases/tag/v0.5.1) 已正式公开，为 latest、非 draft、非 prerelease，于 2026-09-10T05:31:57Z 发布。此前 v0.5.0 发布保留为历史检查点。当前源码改动目标为 `v0.5.2`，已在 `updater` 本地提交为 `ed59f4402b421207456a8530a369b2ac9c6a2a89`，尚未发布；source-0.5.0 候选证据不建立 v0.5.2 游戏运行结论。
...
12. [x] 扩展 GitHub Latest 的热修复更新比较：远端 numeric version core 高于已安装 core 时更新；numeric core 相同但 dash suffix 不同时也更新；远端 core 更低时跳过，忽略 build metadata。编译包含 `windows_http.cpp` 的目标后，`LoUpdaterTest --version-policy` 通过 14 项检查、0 项失败。源码与发布目标保持 `0.5.0`。这只是宿主侧策略验证：未重建或运行游戏 EXE，未进行网络请求、下载 transaction、helper 执行、玩家验收、push、打包或 Release。
