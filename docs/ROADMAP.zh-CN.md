...
12. [x] 扩展 GitHub Latest 的热修复更新比较：远端 numeric version core 高于已安装 core 时更新；numeric core 相同但 dash suffix 不同时也更新；远端 core 更低时跳过，忽略 build metadata。编译包含 `windows_http.cpp` 的目标后，`LoUpdaterTest --version-policy` 通过 14 项检查、0 项失败；本次仅变更目标，未重跑。用户将源码与交付目标设为 `v0.5.1`。这只是宿主侧策略验证：未重建或运行游戏 EXE，未进行网络请求、下载 transaction、helper 执行、玩家验收、打包或 Release。
