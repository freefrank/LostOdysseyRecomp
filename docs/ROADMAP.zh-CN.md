...
12. [x] GitHub Latest 数字版本更高，或数字版本相同但后缀不同时触发更新。后缀策略已在 main 的 `bf549d3`，14 项版本规则检查通过。本地 0.5.1 另已支持在正式安装目录独立启动更新器，校验已安装 EXE 并检查运行中的游戏；Release /MT 定向检查通过 43 项，包括测试文件替换和 probe 启动。该宿主验证未运行真实游戏或下载公开安装包；发布阶段另行完成下载校验。玩家验收待确认。 发布目标为 `v0.5.1-updaterfix`，完整后缀打包的 7 项定向检查通过；已从 `396e7809` 发布 [v0.5.1-updaterfix](https://github.com/freefrank/LostOdysseyRecomp/releases/tag/v0.5.1-updaterfix)，安装包和匿名下载校验通过。
