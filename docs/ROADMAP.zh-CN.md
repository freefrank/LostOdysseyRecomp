...
- [~] **原生 Vulkan DLSS 超分辨率（SR）：**`dlss` 分支正在实施原生 Vulkan NGX DLSS SR（P0–P3，P4 插帧暂缓）。代码提交已推送到 `origin/dlss` 至 P2 检查点 `744ab91`（`744ab911f490355177342da453fdef515be92394`，涵盖 P0 bridge/probe `089676f`、P1 契约/尺寸/计划 `c6bc50b` 与 `3f40030`、P1 文档 `91bf37e`，以及 P2 原生适配器 `d018be7` 与受控渲染器检查点 `744ab91`）。通过 62 项生产 planner 检查与 112 项 Vulkan GPU 输入检查（Gate 2 审查通过）。P2 应用户要求暂停，实现中已闭环 4 项检查点审查修复；重审流程已取消，P2 阶段与 Gate 3 未验收。有界本地验证通过 RTX 5080 4×4 到 8×8 生产 RGBA 重采样（64 像素 0 不匹配，1 字节 alpha 容差）与独立首像素有限非零检查，不构成运动与画质证明；Khronos 验证层不可用。实机 SR 调度仍因未知颜色编码绕过而保持禁用；目标晋升映射、客户机 alpha 保留、帧中 Flush、纹理别名、动态窗口缩放、UI 顺序、原生 Linux 执行、游戏验收与二进制发布均未完成。Issue #10 其他目标仍保持开放。详细验证与交接记录于 [Native DLSS Validation](notes/native-dlss-validation.md) 与 [Native DLSS Handoff](notes/native-dlss-handoff.zh-CN.md)。
