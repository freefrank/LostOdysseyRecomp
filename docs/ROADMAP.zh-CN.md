...
更新于 **2026-09-07**，以 v0.3.0 发布和新增 v0.4.0 开发范围为准。`[x]` 表示所述范围已完成验证，不代表全游戏通关。历史调查记录保留当时状态；当前进度以 [STATUS.md](STATUS.md) 为准。

状态：`[ ]` 计划／待完成 · `[~]` 进行中 · `[x]` 已在所述范围验证。

## 当前开发目标：v0.4.0

此前 v0.4.0 检查点完成了所记录的实现与限定验证：文字／UI、Debug、空间 AA／缩放、实验性 TAA、采样 60 FPS 行为及 DLSS/FSR 可行性研究。更多流程列为回归，无新增用户验收。见[实现检查点与验收标准](notes/v0.4.0-development.md)。v0.4.0 仍在开发，保持 Unreleased 状态，也没有承诺发布日期。
...
用户反馈所有场景均看不出 upscale 改善，此前也反馈切换 TAA 看不出差别，并明确要求真正的内部分辨率最高到 4K，优先跟随输出。既有执行链检查在对应构建范围内继续有效；新构建的可感知改善仍待用户复查。记录中的开发构建以 `c548b48` 为基础并包含后续改动；v0.4.0 仍为 Unreleased。见[后续交接](notes/handoff-v0.4.0-followup.md)。
...
以下保留既有兼容性和回归待办；新增开发范围以上方 v0.4.0 目标为准。
...
- [x] 完成 v0.4.0 的 DLSS/FSR 时域输入可行性研究，涵盖实际证据、缺失输入及未来接入门槛。尚无供应商后端，本地设置页已移除 v0.3.0 的 DLSS 禁用占位。
- [ ] 帧生成 FG 作为后续研究；本地设置页已移除已发布版本中的禁用控件。
