...
- [x] **超宽屏 (21:9) 与 FOV 布局（Issue #17）：**实验性原生 Hor+ 超宽屏渲染已随 v0.6.6 发布（同版本重发自源提交 `c6cbd1f62414a46c00c6312559edcd8d217bc9ee`，Release CI 35527543573；Windows ZIP `bed792e5…` 与 Linux AppImage `4c5ac5e5…` 均通过核对）；超宽屏下已知的阴影映射缺陷已通过无颜色写入 mode 4+5 深度光栅化与缓存高度修正完成修复并交付，并在测试场景中获得用户验收确认。图形设置菜单在输出分辨率上方新增宽屏开关，开启后提供 1720×720、2560×1080、3440×1440、3840×1600、5120×2160 五档 21:9 预设（关闭切回 16:9，按最近高度映射且等距取高档，保持保存／取消原路径，旧宽屏配置自动推导），首启列表与五语言本地化已同步。菜单 hook 流测试（初始 3440 推导、五档正向循环、2160 开关、取消重开、保存应用）、离线渲染截图布局可读性及增量构建均通过（10/10 构建成功）。用户运行最新构建后已验收当前宽屏交付并确认无问题，授权关闭 Issue #17，纳入目标版本 v0.6.7 发布；发布打包与 CI 仍待执行。不宣称全分辨率或跨 GPU 穷尽覆盖，未来若有新回归或场景反馈再行跟进。
