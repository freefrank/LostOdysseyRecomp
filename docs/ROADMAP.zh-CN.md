...
更新于 **2026-09-06**，以 v0.1 和最新用户验收为准。`[x]` 表示所述范围已完成验证，不代表全游戏通关。历史调查记录保留当时状态；当前进度以 [STATUS.md](STATUS.md) 为准。

状态：`[ ]` 计划／待完成 · `[~]` 进行中 · `[x]` 已在所述范围验证。

## 最新里程碑：v0.1

Windows 版本、游戏导入器、首次启动设置、五语言设置、内置着色器资源索引和并行预编译已发布，包含近期对白、脚下投影、海报和窗口响应修复。见[发布说明](RELEASE-v0.1.md)。
...
- [x] 确认支持的 XEX 为版本 4、无 TU、亚洲多语言版本；按可执行文件详情和哈希匹配，不仅依赖区域标签。见 [XEX](notes/xex.md)。
...
- [ ] 验证四盘运行时整合和路径重定向；四盘导入已完成，后续盘推进与换盘尚未确立。
- [ ] 逐一验证过场、战斗、千年之梦和大地图。
...
- [~] Map 13 人物表面阴影：用户反馈改善并满意，受控回归待做，尚未证明与海报同因。
- [ ] 火焰受击黑红格子，以原机画面为参考。
...
- [ ] 研究 DLSS/FSR 等超分辨率；当前输出缩放并非时域超分，v0.1 的 DLSS 为禁用占位。
- [ ] 研究帧生成 FG；v0.1 为禁用占位。
