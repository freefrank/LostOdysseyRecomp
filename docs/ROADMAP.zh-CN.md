...
- [~] **发布流程复用 PPC 库：**release workflow 现默认恢复预编译 PPC 静态库，并保留 `rebuild_ppc=true` 源码构建回退。导出、恢复和校验 CLI 与 CMake 导入支持已实现。13 个合成 fixture、两份 workflow 的 actionlint、真实 138,454,798-byte PPC 库导出（SHA-256 `ba3e4c4dff009d6d8e844c007186a6e5040266875bca6423f8fe26f8d27fb21b`）、restore/CMake target 检查及 runtime Ninja graph 检查均通过。该 graph 含零条 PPC compile command，且在校验依赖之后链接导入库。私有 build-inputs commit `77f076e0e03966736cbf8919ce793bafadce82d9` 已推送，GitHub API 已读回匹配的五个 blob；这只是构建产物传输，不是公开发布。托管 CI、主仓库新提交／推送、Release 发布和玩家验收仍待完成。
