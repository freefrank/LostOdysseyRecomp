...
更新于 **2026-09-08**，涵盖 v0.4.1 发布状态。`[x]` 表示所述范围已完成验证，不代表全游戏通关。历史调查记录保留当时状态；当前进度以 [STATUS.md](STATUS.md) 为准。
...
- [ ] 由 Issue #7 原报告者使用其存档与硬件确认结果；本机复现不证明与其未记录异常完全相同。
...
- [ ] 完成最终 v0.4.2 CI 构建和包身份／启动路径核验，单独记录实际发布产物的结果。
