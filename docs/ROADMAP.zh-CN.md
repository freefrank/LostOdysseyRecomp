...
- [~] **超宽屏 (21:9) 与 FOV 布局（Issue #17）：**实验性原生 Hor+ 超宽屏渲染已随 v0.6.6 发布（同版本重发自源提交 `c6cbd1f62414a46c00c6312559edcd8d217bc9ee`，Release CI 35527543573；Windows ZIP `bed792e5…` 与 Linux AppImage `4c5ac5e5…` 均通过摘要核对与公开 HTTP 200 下载检查）。实验范围当前仅支持 3440×1440 分辨率。Windows Clang 构建通过；`FramePlan`（18 项检查）、`TargetMapping`（7 项检查）、分辨率计算（40 项检查）及时序抖动/相位测试均通过。Vulkan 内置 28,482 个着色器启动并在 13.93 秒加载存档；场景转换截图 382/383 以及主颜色/深度 3440×1472（含 padding）resolve 至 3440×1440 均获验证。超宽屏下已知的阴影映射缺陷已通过无颜色写入 mode 4+5 深度光栅化与缓存高度修正完成修复并交付，运行时构建与 TargetMapping 7 项通过，并在测试场景中获得用户验收确认；此前不相关的 CPU 异常报告已被撤回（确认为宿主环境干扰）。完整稳定实机游玩、Hor+ 画面视觉、HUD、动态切分辨率、故障注入、其它分辨率以及其它后端验证仍待用户手动测试与视觉验收；Issue #17 保持打开且处于进行中状态。
