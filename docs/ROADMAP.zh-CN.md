...
- [~] **原生 Vulkan DLSS 超分辨率（SR）：**Windows RTX 5080 上的开发与定向执行已获得界定证据。窄范围 SDR producer-resolve qualification、CPU 有界测试、720p/1440p 持久会话尺寸、14,720 像素同尺度 color-height growth、motion cold-start 共享 helper 和显式 `Pending` fallback recovery 均通过记录范围；恢复日志含 38 项 GPU 检查。生产 SR 已确认 Quality 1707×960 到 2560×1440、`DisplayEncoded` 色彩和反转 Z 深度。Create=1、Evaluate=24 是 128 次调用记录上限内的 retained success record，不是总次数；isolated use 10684 已在 serial 23614 提交，并在快照时仍处于 in flight；23612 是较早的 completed watermark。未记录 failure、device loss 或 request disable。该运行只进入场景，未宣称视觉验收。P3/Gate 3 尚未全部验收，尚未发布；原生 Linux 执行和 SDK 分发仍未解决。待处理还包括 `LO_NO_RENDERER` shutdown drain 和旧 execution fixture 的 unchecked void-submit-wait。Issue #10 其他目标仍保持开放。详细验证与交接记录于 [Native DLSS Validation](notes/native-dlss-validation.md) 与 [Native DLSS Handoff](notes/native-dlss-handoff.zh-CN.md)。
