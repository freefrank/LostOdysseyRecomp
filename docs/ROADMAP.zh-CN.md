...
更新于 **2026-09-06**，以 v0.2.2 和最新用户验收为准。`[x]` 表示所述范围已完成验证，不代表全游戏通关。历史调查记录保留当时状态；当前进度以 [STATUS.md](STATUS.md) 为准。
...
- [~] 准备用户授权的commit／push／发布；CI和正式包验证待完成，不隐含新增玩家画面／卡顿验收。
...
2. [~] 在v0.3.0交付扩展shader及已记录管线准备，再收集玩家验收。本地验证证明来源覆盖和实际draw复用，不代表完全消除卡顿或覆盖全新场景首用PSO。
