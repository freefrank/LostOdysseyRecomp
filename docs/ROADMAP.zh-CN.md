...
原定 v0.4.0 实现与限定验证已完成：文字／UI、Debug、空间 AA／缩放、实验性 TAA、采样 60 FPS 行为及 DLSS/FSR 可行性研究。更多流程列为回归，无新增用户验收。见[实现检查点与验收标准](notes/v0.4.0-development.md)。v0.4.0 仍为本地开发，未推送／未发布，也没有承诺发布日期。

1. [x] 通过按输出分辨率绘制宿主设置页及支持场景的 UI 前 AA 改善文字清晰度。同一真实 Navigation 帧重放、生产 GPU 控制和 v7 最终 AA 跳过记录，验证后续 UI 移除了重复 AA；实际七行图形页布局也已通过。guest UI 仍为 720p；不支持场景／CPU 回退及更多字形／语言质量列为独立回归覆盖。
...
5. [x] 自动匹配实际输出并提供标准／双线性与高／双三次质量：选定 GPU／配置及真实帧重放检查通过。实际七行图形页验证了预览不写盘、15 秒恢复 High、Keep 保存 Standard 及同进程关闭／重开。guest 尺寸保持固定；可调比例属于独立研究，进程重启／显示覆盖列为回归。
...
- [x] 为 v0.4.0 增加自动输出分辨率匹配和标准／高质量：选定检查及实际七行图形页预览／回退／Keep／同进程重开通过；可调 guest 渲染比例保留为独立研究。
